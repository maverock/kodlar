...
def SaltKeyAdd( GetIp,GetInstance ):
    ...
    Log(subprocess.check_output(cmd1.split()))
    Log(subprocess.check_output(cmd2.split()))

def SaltKeyDelete( GetIp ):
    saltid="ip-"+GetIp.replace(".","-")
    cmd="sudo salt-key -d "+saltid
    Log(subprocess.check_output(cmd.split()))
